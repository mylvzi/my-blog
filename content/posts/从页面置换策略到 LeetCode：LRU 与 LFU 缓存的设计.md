...
今天看《操作系统导论》的页面置换策略章节，顺手把 LeetCode 上的 LRU Cache 和 LFU Cache 两道题写了一遍。写完之后有一个比较直接的感受：**这两个题表面上是数据结构题，本质上是在用工程化的数据结构模拟操作系统里的缓存管理策略。**

* **缓存问题的核心不是“存不存”，而是“满了以后删谁”。**

在虚拟内存系统中，进程看到的是一个巨大的虚拟地址空间，但物理内存是有限的。虚拟页并不总是全部驻留在物理内存中，一部分可能在磁盘上。当发生缺页异常时(Page Fault)，操作系统需要把目标页调入内存(Swap in)。如果此时物理内存已经满了，就必须选择一个已有页面换出去(Swap out)。

这就是页面置换策略要解决的问题。

理论上，最优置换算法是：**淘汰未来最晚才会被访问的页面**, 这个策略能最大化未来的缓存命中率。但问题是，操作系统无法预知未来(就像 CPU 调度时不知道任务的工作时间一样)。所以现实系统只能基于历史访问记录做预测(有没有想到 CPU 调度的MLFQ?)。

历史经验大致有两个维度：
* 第一，近期性。越是最近被访问过的页面，越可能很快再次被访问。
* 第二，频率。越是经常被访问的页面，越可能继续被访问。

LRU 利用的是**近期性**，LFU 利用的是**频率**。二者都是局部性原理在缓存管理中的具体体现。
...
LFU，全称 Least Frequently Used，意思是淘汰访问**频率**最低的元素。
...
这两道题有一个值得注意的地方：LeetCode 要求的是接口级别的 `O(1)` 操作，因此可以使用 HashMap 和双向链表进行精确维护。

但操作系统面对的是完全不同的约束。

在操作系统中，每一次内存访问都可能非常频繁。如果为了实现精确 LRU，每次访问页面都要移动链表节点，那么这个成本会非常高。更现实的方案是维护页表项中的 use bit、dirty bit 等状态位，用硬件协助记录访问情况，再由操作系统周期性扫描。

所以算法题里的 LRU 更像是一个抽象模型：
...
LRU 和 LFU 都是缓存淘汰策略，本质上是在有限容量下利用历史信息预测未来访问。
* LRU 利用近期性，认为最近访问过的数据更可能再次访问。
* LFU 利用频率，认为访问次数更多的数据更可能继续访问。
...
其中 LFU 需要额外维护 `minFreq`，并且在频率相同时退化为 LRU。

附上一张 GPT-image 2 生成的总结图~
![](https://img.wemd.app/1777214719405_8oldnx.jpg)附上一张GPT-image2生成的总结图~

附录:
LRU 代码, 链接: [146. LRU 缓存 - 力扣（LeetCode）](https://leetcode.cn/problems/lru-cache/)
```java
class LRUCache {
    // 虚拟内存的知识
...
LFU 代码,链接: [460. LFU 缓存 - 力扣（LeetCode）](https://leetcode.cn/problems/lfu-cache/submissions/721326851/)
```java
import java.util.*;
