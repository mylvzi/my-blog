<#
.SYNOPSIS
  Sync practice logs from local directory to Hexo blog.
  Scan E:\lsq_learn\事上磨炼日志\ for new .md files and import them as practice logs.

.DESCRIPTION
  - Scans E:\lsq_learn\事上磨炼日志\<month-dir>\*.md
  - Parses date from filename (2026-6-1.md -> 2026-06-01)
  - Creates Hexo post with practice_log front matter
  - Tracks imported files in .practice-log-tracker.json to avoid duplicates
  - Automatically runs deploy.bat to push to remote

.EXAMPLE
  pwsh -ExecutionPolicy Bypass -File .\tools\sync-practice-log.ps1
#>
...
$sourceDir = "E:\lsq_learn\事上磨炼日志"
$trackerFile = Join-Path $repoRoot ".practice-log-tracker.json"
...
function New-PracticeFrontMatter([string]$DisplayDate, [string]$FormattedDate) {
  $now = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
  return @"
---
title: "$DisplayDate"
date: $FormattedDate
permalink: practice/$FormattedDate/
practice_log: true
tags:
  - 事上磨练日志
comment: true
...
$monthDirs = Get-ChildItem -LiteralPath $sourceDir -Directory -ErrorAction SilentlyContinue
foreach ($monthDir in $monthDirs) {
  ...
  foreach ($file in $mdFiles) {
    ...
    $relativeKey = $sourcePath.Replace($sourceDir, "").TrimStart("\").Replace("\", "/")
    ...
    $postFileName = "$($dateInfo.DateFormatted)-practice-log.md"
    ...
    # Build the post
    $frontMatter = New-PracticeFrontMatter -DisplayDate $dateInfo.Display -FormattedDate $dateInfo.DateFormatted
...
if ($importedCount -eq 0 -and $updatedCount -eq 0) {
  Write-Host "No new or updated logs. Done."
  exit 0
}
