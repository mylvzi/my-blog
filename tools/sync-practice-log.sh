#!/usr/bin/env bash
#
# sync-practice-log.sh — Sync practice logs to Hexo blog (macOS version).
#
# Usage:
#   bash tools/sync-practice-log.sh
#   PRACTICE_SOURCE_DIR=/custom/path bash tools/sync-practice-log.sh
#
set -euo pipefail
...
PRACTICE_SOURCE_DIR="${PRACTICE_SOURCE_DIR:-/Volumes/KINGSTON/lsq_learn/事上磨炼日志}"
TRACKER_FILE="$REPO_ROOT/.practice-log-tracker.json"

if [ ! -d "$PRACTICE_SOURCE_DIR" ]; then
  echo "ERROR: Practice log source directory not found: $PRACTICE_SOURCE_DIR"
  echo "Set PRACTICE_SOURCE_DIR env var to override."
  exit 1
fi

echo "========================================="
echo "Syncing Practice Logs"
echo "Source : $PRACTICE_SOURCE_DIR"
echo "========================================="

# Run Python — status to stderr, results to stdout
RESULT=$(python3 - "$PRACTICE_SOURCE_DIR" "$POSTS_DIR" "$TRACKER_FILE" 2>&1 << 'PYEOF'
...
if [ "$HAS_CHANGES" = false ]; then
  echo "No new or updated logs. Done."
  exit 0
fi
...
echo ""
echo "===== Deploying ====="
bash "$REPO_ROOT/deploy.sh" --skip-build "sync: practice logs $(date '+%Y-%m-%d')"
echo ""
